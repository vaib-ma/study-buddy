...
function QuestionCard({ question, index, solution, loading, onSolution }) {
  ...
  return (
    <article className="question-card card">
      ...
      <MathText className="question-text">{question.question_text || question.question}</MathText>
        ...
        <div className="options">
          {options.map((option, i) => (
            <div className="option" key={i}>
              ...
              <MathText className="option-text">{typeof option === 'string' ? option : option.text}</MathText>
            </div>
          ))}
        </div>
      )}
    ...
    <div className="solution">
      ...
      {solution.final_answer && (
        <div className="final-answer">
          <strong>Answer:</strong> <MathText className="answer-math">{solution.final_answer}</MathText>
        </div>
      )}
      {explanation && (
        <MathText className="solution-explanation">
  ...
  return (
    <section className="test-shell">
      ...
      <article className="test-question card">
        ...
        <MathText className="test-question-text">{q.question_text || q.question}</MathText>
...
function normalizeMathBody(value) {
  return String(value ?? '')
    .replace(/\\_/g, '_')
    .replace(/\\\^/g, '^')
    .replace(/(^|[^\\])\\?sin(?=(?:\\?\^|[0-9a-zA-Z{]))/g, '$1\\sin ')
    .replace(/(^|[^\\])\\?cos(?=(?:\\?\^|[0-9a-zA-Z{]))/g, '$1\\cos ')
    .replace(/(^|[^\\])\\?tan(?=(?:\\?\^|[0-9a-zA-Z{]))/g, '$1\\tan ')
    .replace(/(^|[^\\])\\?cot(?=(?:\\?\^|[0-9a-zA-Z{]))/g, '$1\\cot ')
    .replace(/(^|[^\\])\\?sec(?=(?:\\?\^|[0-9a-zA-Z{]))/g, '$1\\sec ')
    .replace(/(^|[^\\])\\?csc(?=(?:\\?\^|[0-9a-zA-Z{]))/g, '$1\\csc ')
    .replace(/\\sum\\_/g, '\\sum_')
    .replace(/\\prod\\_/g, '\\prod_');
}
...
function normalizeMathText(value) {
  ...
  // Repair common extraction damage inside $...$ / $...$ / \\(...\\) / \\[...\\].
