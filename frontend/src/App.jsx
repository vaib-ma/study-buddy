...
function normalizeMathBody(value) {
  ...
  for (const [name, latex] of functions) {
    const pattern = new RegExp('(^|[^\\\\])' + name + '(?=(?:\\\\^|[0-9a-zA-Z{]))', 'g');
    text = text.replace(pattern, '$1' + latex + ' ');
  }

  text = text.replace(/\\sum\\_/g, '\\sum_');
  text = text.replace(/\\prod\\_/g, '\\prod_');
  return text;
}
